...
for (let y = 0; y < matrix.length; y++) {
  for (let x = 0; x < matrix[0].length; x++) {
    ...
    if (y + 1 < matrix.length) {
      adjacencyList
        .get(key)
        .push({ node: `${x},${y + 1}`, direction: directions.DOWN });
    }
    if (x + 1 < matrix[0].length) {
      adjacencyList
        .get(key)
        .push({ node: `${x + 1},${y}`, direction: directions.RIGHT });
    }
    if (y - 1 >= 0) {
      adjacencyList
        .get(key)
        .push({ node: `${x},${y - 1}`, direction: directions.UP });
    }
    if (x - 1 >= 0) {
      adjacencyList
        .get(key)
        .push({ node: `${x - 1},${y}`, direction: directions.LEFT });
    }
  }
}

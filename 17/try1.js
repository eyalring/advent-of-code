...
function build_graph(range_min, range_max) {
  for (let col = 0; col < inputs.length; col++)
    for (let row = 0; row < inputs[0].length; row++) {
      ...
      graph[col + "," + row + "_v"] = JSON.parse(
        '{"least":999999,"neighbors":{}}'
      );
      graph[col + "," + row + "_h"] = JSON.parse(
        '{"least":999999,"neighbors":{}}'
      );
...
function walk() {
  ...
  Object.assign(
    starting_obj,
    graph["0,0_h"].neighbors,
    graph["0,0_v"].neighbors
  );
  ...
  for (let i in queue)
    queue[i] = [queue[i], queue[i].match("h") ? "0,0_v" : "0,0_h"];
  //We set length to neighbors from starting pos
  for (let i in queue)
    graph[queue[i][0]].least =
      graph[queue[i][1]].least + graph[queue[i][1]].neighbors[queue[i][0]];
...
console.log(
  "part 1 answer: " +
    Math.min(graph["140,140_h"].least, graph["140,140_v"].least)
);
//reset stuff
graph = {};
starting_obj = {};

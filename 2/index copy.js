const input = `Game 1: 1 blue, 2 green, 3 red; 7 red, 8 green; 1 green, 2 red, 1 blue; 2 green, 3 red, 1 blue; 8 green, 1 blue
...
const find = `12 red cubes, 13 green cubes, and 14 blue cubes`;

const mapToFind = { red: 12, green: 13, blue: 14 };

const result = input.split("\n").reduce((acc, line) => {
  let valid = true;
  const mainParts = line.split(":");
  const gameId = parseInt(mainParts[0].split(" ")[1]);
  const subGames = mainParts[1].split(";");
  console.log(subGames);
  subGames.forEach((subGame) => {
    const colors = subGame
      .trim()
      .split(",")
      .forEach((colorAndNumber) => {
        const colorAndNumberArray = colorAndNumber.trim().split(" ");
        const ballsNumber = parseInt(colorAndNumberArray[0]);
        const color = colorAndNumberArray[1];

        console.log({ color, ballsNumber });

        if (ballsNumber > 16) {
          console.log("invalid");
        }

        if (mapToFind[color] < ballsNumber) {
          valid = false;
        }
      });
  });
